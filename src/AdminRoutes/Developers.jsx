...
const Developers = () => {
  ...
  const fetchDevelopers = async () => {
    try {
      const res = await api.get("/admin/collaboratorRoute/collaborator");
  ...
  const handleCreateDeveloper = async (e) => {
    e.preventDefault();
    try {
      const res = await api.post("/auth/registerdeveloper", newDev);
  ...
  return (
    <div className="p-6 min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 space-y-8">
      ...
      >
        <h2 className="text-2xl font-bold mb-4 text-gray-800 text-center">Developer List</h2>
        ...
        <form onSubmit={handleCreateDeveloper} className="space-y-4">
          {["name", "email", "password", "role"].map((field) => (
            <input
              ...
              className="w-full border border-gray-300 rounded-xl px-4 py-2 focus:ring-2 focus:ring-purple-400 focus:border-purple-400 outline-none transition-all"
              required
            />
          ))}
          <button
